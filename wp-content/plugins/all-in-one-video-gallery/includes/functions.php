...
function aiovg_get_image( $object_id, $size = "large", $object_type = "post", $placeholder_image = false ) {
	...
	if ( 'term' == $object_type ) { 
		...
		$attach_id = get_term_meta( $object_id, 'image_id', true );
	} else { 
		// Get image from posts
		$attach_id = get_post_meta( $object_id, 'image_id', true );
		...
		if ( empty( $attach_id ) ) {
			...
			if ( ! empty( $image_url ) ) {
				...
				$attach_id = get_post_meta( $object_id, '_thumbnail_id', true );
			}
		}
	}

	if ( ! empty( $attach_id ) ) {
		$attributes = wp_get_attachment_image_src( (int) $attach_id, $size );
		if ( ! empty( $attributes ) ) {
			$image_data['src'] = $attributes[0];
		}

		$image_data['alt'] = get_post_meta( (int) $attach_id, '_wp_attachment_image_alt', true );
	}
